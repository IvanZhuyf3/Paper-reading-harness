# Li et al. 2026 - Fourier-transform Overtone Photothermal Spectromicroscopy

## Paper identity

- **Title:** Supercontinuum Fourier-transform Overtone Photothermal Spectroscopy and Microscopy
- **Authors:** Geng Li et al.
- **Target journal:** Advanced Photonics
- **Manuscript ID:** AP-26-149008
- **Submission date shown in proof:** 22-Aug-2026
- **DOI:** not assigned in the supplied proof

## Source integrity

- **Local source:** `source/Li_et_al_2026_FT_OPT_proof.pdf`
- **PDF pages:** 43
- **Main article:** PDF pp. 2-29 (article pp. 1-28)
- **Embedded Supplementary Information:** PDF pp. 30-43
- **Bytes:** 12,531,864
- **SHA-256:** `702F012A0C7FBA10ECDB9DC03619D3AF9CD8DFF05B288F52A31911C6142A6F6D`
- **Copy verified against supplied file:** YES

The source PDF is treated as immutable. It is a review proof rather than a final
published version.

## Workspace roles

- `source/`: copied source material.
- `model/`: pending paper model plus mechanical and semantic audits.
- `sessions/`: future TRAINING or EFFICIENT_READING sessions.
- `artifacts/`: extracted text, page renders, contact sheets, and inspection metadata.

## Current status

- Page-delimited text and all 43 page renders generated.
- Eight contact sheets visually inspected; see `artifacts/inspection_manifest.md`.
- Paper model: `model/paper_model.pending.toml`, version 0.1.0.
- Paper-model SHA-256: `AF997481BA7FC92462E3750AC5FD14020091A25BDC02665968EF68A0F8A575B9`.
- Selection policy: 1.2.
- Mechanical audit: `model/paper_model.audit.md`, 20/20 checks passed.
- Semantic audit: `model/paper_model.semantic_audit.md`.
- Human approval: not reviewed.
- Curriculum status: not approved for reuse.
- Active session: `sessions/2026-08-28_foundation_training.state.toml`.
- Runner/level: TRAINING / FOUNDATION.
- Session initialization audit: 38/38 checks passed.
- Current cursor: `KNOWLEDGE.K1.await_response`.
