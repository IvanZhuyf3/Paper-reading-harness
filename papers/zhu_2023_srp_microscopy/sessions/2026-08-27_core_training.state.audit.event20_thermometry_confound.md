...
| Completed stages form the prefix before the current stage | PASS | completed=['knowledge', 'idea', 'claims'], expected=['knowledge', 'idea', 'claims'] |
| Asked IDs are unique | PASS | count=21 |
...
| Unique next interaction is persisted | PASS | cursor=EVIDENCE.M1.awaiting_bleach_control_implementation, pending_prompt=EVIDENCE_M1_BLEACH_ONLY_CONTROL_IMPLEMENTATION |
| Event sequence is contiguous | PASS | events=20 |
